...
void hook_BroadcastVoiceData(IClient* cl, uint nBytes, char* data, int64 xuid) {
	...
	if (afflicted_players.find(uid) != afflicted_players.end()) {
		...
		case AudioEffects::EFF_PROOT:
			...
			break;
		case AudioEffects::EFF_CUSTOM:
			AudioEffects::CustomFilter((uint16_t*)&decompressedBuffer, samples, uid);
			break;
		default:
			break;
		}
...
LUA_FUNCTION_STATIC(eightbit_setcustomeffect) {
	int id = LUA->GetNumber(1);
	float pitch = LUA->GetNumber(2);
	float lowpass = LUA->GetNumber(3);
	float reverb = LUA->GetNumber(4);
	AudioEffects::customParamsMap[id] = {pitch, lowpass, reverb};
	return 0;
}
...
GMOD_MODULE_OPEN()
{
	...
	LUA->CreateTable();
		...
		LUA->PushString("SetCustomEffect");
		LUA->PushCFunction(eightbit_setcustomeffect);
		LUA->SetTable(-3);
		...
		LUA->PushString("EFF_MASKVOICE");
		LUA->PushNumber(AudioEffects::EFF_MASKVOICE);
		LUA->SetTable(-3);
		...
		LUA->PushString("EFF_CUSTOM");
		LUA->PushNumber(AudioEffects::EFF_CUSTOM);
		LUA->SetTable(-3);
	LUA->SetTable(-3);
